...
def test__gitignore_does_not_exist() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_does_not_exist", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
...
def test__gitignore_is_not_empty() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_is_not_empty", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
        assert test_task.execute().is_failed()
...
def test__gitignore_has_unmodified_hash() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_has_unmodified_hash", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
        assert test_task.execute().is_failed()
...
def test__gitignore_has_unmodified_content() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_has_unmodified_content", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
        assert test_task.execute().is_failed()
...
def test__gitignore_has_correct_tokens() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_has_correct_tokens", GitignoreCheckTask, group="check", tokens=["test"], extra_paths=[]
        )
        assert test_task.execute().is_failed()


def test__gitignore_has_correct_extra_paths() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        project.directory = VALID_GITIGNORE_DIR
        test_task = project.do(
            "test__gitignore_has_correct_extra_paths",
            GitignoreCheckTask,
            group="check",
            tokens=[],
            extra_paths=["test"],
...
def test__gitignore_is_sorted() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        ...
        test_task = project.do(
            "test__gitignore_is_sorted", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
        assert test_task.execute().is_failed()
...
def test__gitignore_passes_when_valid() -> None:
    with kraken_ctx() as ctx, kraken_project(ctx) as project:
        project.directory = VALID_GITIGNORE_DIR
        test_task = project.do(
            "test__gitignore_passed_when_valid", GitignoreCheckTask, group="check", tokens=[], extra_paths=[]
        )
